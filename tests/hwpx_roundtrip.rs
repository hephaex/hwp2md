...
///
/// Each test follows pipeline (A): MD → IR → HWPX bytes → read HWPX → IR → verify structure.
///
/// Full-roundtrip tests (MD → HWPX → IR → MD, Phase D-1) live in
...
use common::{collect_all_text, first_blocks, md_to_hwpx_to_ir};
...
// Phase D-1 full-roundtrip tests have been moved to hwpx_roundtrip_full.rs.
