...
Sprint 56 완료 (2026-05-21). 다음 스프린트 우선순위:
...
- **P2**: `style_id` 필드 타입 `u16` → `u8` 변경 (Sprint 56 리뷰 M1). 안전: 스펙상 0-255 범위. dispatcher.rs 읽기 코드 + 모든 `style_id: 0` 리터럴 업데이트.
- **P3**: `is_heading_terminator` 단위 테스트 추가 (Sprint 56 리뷰 L2) — 정책을 명시적으로 고정.
