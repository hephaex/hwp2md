...
Sprint 55 완료 (2026-05-21). 다음 스프린트 우선순위:

- **P1**: 편(part, 장보다 상위) 감지 — 대형 법전(민법, 형법)에 필요. 필요 픽스처: `tests/fixtures/real/` 아래 편-포함 HWP 파일 (민법/형법/상법 등). 편 추가 시 장→H2, 절/조→H3 shift 필요 (기존 황금 파일 모두 재생성 필요).
- **P2**: M2 (review 미해결) — suffix 경계 검사 (`제3장은`, `제3장에서` 같은 인라인 참조 오승급). suffix 직후 문자가 공백/괄호/구두점이 아니면 None 반환.
- **P3**: PARA_HEADER 오프셋 검증 — pyhwp/hwplib 스펙 대조 (4회 연기)
- **P4**: 기사 제목 vs 기사 본문 분리 — 일부 문서는 기사 제목만 제N조(제목)으로 분리된 별도 단락을 가짐; 현재 100자 상한은 합리적 근사치지만 완벽하지 않음
