...
Sprint 57 완료 (2026-05-21). 다음 스프린트 우선순위:
...
- **P2**: `is_heading_terminator` 차단 집합 확장 (Sprint 57 리뷰 LOW-1) — `%`, `&`, `/`, `"` 비한국어 구두점 추가 + 테스트.
- **P3**: `is_heading_terminator` 허용 집합 누락 대표값 추가 (Sprint 57 리뷰 LOW-2) — `[`, `]`, `：`, `．`, `；` 테스트 보강.
